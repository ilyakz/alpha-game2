...
from cube_tic_tac_toe.CubeTicTacToeLogic import Board
...
class CubeTicTacToeGame(Game):
    ...
    def getEstimatePos(self, board, player):
        ...
        for z in range(n):
            for y in range(n):
                for x in range(n):
                    if b.pieces[z * n + y][x] == 0:
                        valid_moves_pl1[z * n * n  + y * n + x ] = +(sum([b.pieces[z * n + j][x] == player for j in range(n)]) + \
                            sum([b.pieces[z * n + y][i] == player for i in range(n)]) + \
                            sum([b.pieces[k * n + y][x] == player for k in range(n)]) + \
                            ((n - 1 - y) == x) * sum([b.pieces[z * n + (n - 1 - i)][i] == player for i in range(n)]) +\
                            (y == x) * sum([b.pieces[z * n + i][i] == player for i in range(n)]) + \
                            (z == y) * sum([b.pieces[k * n + k][x] == player for k in range(n)]) + \
                            (z == (n - 1 - y)) * sum([b.pieces[k * n + (n - 1 - k)][x] == player for k in range(n)]) + \
                            (z == x) * sum([b.pieces[k * n + y][k] == player for k in range(n)]) + \
                            (z == (n - 1 - x)) * sum([b.pieces[k * n + y][n - 1 - k] == player for k in range(n)]) + \
                            (z == y == x) * sum([b.pieces[k * n + k][k] == player for k in range(n)]) + \
                            (z == y == (n - 1 - x)) * sum([b.pieces[k * n + k][n - 1 - k] == player for k in range(n)]) + \
                            (z == (n - 1 - y) == x) * sum([b.pieces[k * n + (n - 1 - k)][k] == player for k in range(n)]) + \
                            (z == (n - 1 - y) == (n - 1 - x)) * sum([b.pieces[k * n + (n - 1 - k)][n - 1 - k] == player for k in range(n)]))
                        valid_moves_pl2[z * n * n  + y * n + x ] = +(sum([b.pieces[z * n + j][x] == -player for j in range(n)]) + \
                            sum([b.pieces[z * n + y][i] == -player for i in range(n)]) + \
                            sum([b.pieces[k * n + y][x] == -player for k in range(n)]) + \
                            ((n - 1 - y) == x) * sum([b.pieces[z * n + (n - 1 - i)][i] == -player for i in range(n)]) +\
                            (y == x) * sum([b.pieces[z * n + i][i] == -player for i in range(n)]) + \
...
#тут у нас сами правила
 #требует доработки
		...
		"""	
        if any([any([sum([b.pieces[k * n + j][i] for i in range(n)]) == self.n for j in range(n)]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + j][i] for j in range(n)]) == self.n for i in range(n)]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + j][i]for k in range(n)]) == self.n for i in range(n)]) == 1 for j in range(n)]) or \
                any([any([sum([b.pieces[k * n + i][i] for i in range(n)]) == self.n]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + (n - 1 - i)][i] for i in range(n)]) == self.n]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + k][i] for k in range(n)]) == self.n]) == 1 for i in range(n)]) or \
                any([any([sum([b.pieces[k * n + (n - 1 - k)][i] for k in range(n)]) == self.n]) == 1 for i in range(n)]) or \
                any([any([sum([b.pieces[k * n + i][k] for k in range(n)]) == self.n]) == 1 for i in range(n)]) or \
                any([any([sum([b.pieces[k * n + i][(n - 1 - k)] for k in range(n)]) == self.n]) == 1 for i in range(n)]) or \
                sum([b.pieces[k * n + k][k] for k in range(n)]) == self.n or \
                sum([b.pieces[k * n + k][(n - 1 - k)] for k in range(n)]) == self.n or \
                sum([b.pieces[k * n + (n - 1 - k)][k] for k in range(n)]) == self.n or \
                sum([b.pieces[k * n + (n - 1 - k)][(n - 1 - k)] for k in range(n)]) == self.n :
            status = 1
        elif any([any([sum([b.pieces[k * n + j][i] for i in range(n)]) == -self.n for j in range(n)]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + j][i] for j in range(n)]) == -self.n for i in range(n)]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + j][i] for k in range(n)]) == -self.n for i in range(n)]) == 1 for j in range(n)]) or \
                any([any([sum([b.pieces[k * n + i][i] for i in range(n)]) == -self.n]) == 1 for k in range(n)]) or \
                any([any([sum([b.pieces[k * n + (n - 1 - i)][i] for i in range(n)]) == -self.n]) == 1 for k in range(n)]) or \
        ...
        if status != 1e-4:
            return status
        if any([b.pieces[k * n + i][j] == 0 for k in range(self.n) for i in range(self.n) for j in range(self.n)]):
            status = 0
        return status
