...
class CubeTensorNNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            self.input_boards = tf.placeholder(tf.float32, shape=[None, self.board_z * self.board_y, self.board_x])    # s: batch_size x board_x x board_y
            self.dropout = tf.placeholder(tf.float32)
            self.isTraining = tf.placeholder(tf.bool, name="is_training")

            x_image = tf.reshape(self.input_boards, [-1, self.board_z * self.board_y, self.board_x, 1])                    # batch_size  x board_x x board_y x 1
            ...
            h_conv4_flat = tf.reshape(h_conv4, [-1, args.num_channels*(self.board_z* self.board_y-2)*(self.board_x-2)])
